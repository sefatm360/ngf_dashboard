...
const SeeAllGigOrdrers = () => {
  ...
  useEffect(() => {
    ...
    let uri = '/out/api/gig-order/get/all';
    let pageNumber = 0;
    if (type) {
      if (type === 'all') {
        uri = '/out/api/gig-order/get/all';
      } else {
        uri = `/out/api/gig-order/get/all/by-status/${type}`;
      }
    }
  ...
  const paginate = async (pageNumber) => {
    if (type) {
      navigate(`/gig-orders/all?type=${type}&page=${pageNumber}`);
    } else {
      navigate(`/gig-orders/all?page=${pageNumber}`);
    }
  };

  const handleType = (value) => {
    navigate(`/gig-orders/all?type=${value}`);
  };
...
export default SeeAllGigOrdrers;
